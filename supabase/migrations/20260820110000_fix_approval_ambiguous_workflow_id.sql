-- Sync migration: already applied live. The first real approval ever made
-- failed with `column reference "workflow_id" is ambiguous`: the function's
-- RETURNS TABLE declares workflow_id, so the bare column reference in the
-- queue-message bookkeeping UPDATE was ambiguous. The path had never run
-- before because model generation was disabled from day one. The UPDATE now
-- aliases the table; nothing else changes.
