-- Completes the synchronous Firecrawl-first path through the same candidate
-- persistence gate as Browser Use, without inventing an external provider ID
-- or leaving a pollable task behind. The temporary identifier exists only
-- inside this transaction and is removed before commit.
create function public.internal_record_synchronous_search_results(
  p_workflow_id uuid,
...
begin
  ...
  if v_queue_message_id is not null then
    perform public.internal_archive_queue_message('retailer_search', v_queue_message_id);
  end if;

  update private.workflow_events
  set actor = 'system',
      metadata = metadata || jsonb_build_object('completedBy', 'firecrawl')
  where id = (
    select max(id)
    from private.workflow_events
    where workflow_id = p_workflow_id
      and event_type = 'search.ready_for_approval'
  );
